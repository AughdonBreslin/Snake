...
def test_straight_move_shifts_head_and_drops_tail(game):
    ...
    assert list(game.snake) == [(4, mid), (3, mid), (2, mid)]
...
def test_walking_into_the_top_wall_ends_the_game(game):
    game.env.food = (10, 5)
    game.env.snake = deque([(5, 1)])
    game.env._occupied = set(game.env.snake)
    game.env.direction = UP
    game.move_queue = []
    over = game.move()
    assert over is True
...
def test_new_food_never_lands_on_the_snake(game):
    for _ in range(200):
        assert game.env._spawn_food() not in game.snake
...
def test_head_may_enter_the_cell_the_tail_is_vacating(game):
    ...
    # collision check. This is snake/env.py's step() and it is the single most
    # load bearing rule in the game.
    game.env.snake = deque([(5, 5), (6, 5), (6, 6), (5, 6)])
    game.env._occupied = set(game.env.snake)
    game.env.direction = DOWN
    game.move_queue = []
    game.env.food = (12, 12)
    over = game.move()
    assert over is False
...
def test_running_into_the_body_ends_the_game(game):
    ...
    game.env.snake = deque(
        [(5, 5), (4, 5), (3, 5), (3, 6), (4, 6), (5, 6), (6, 6)]
    )
    game.env._occupied = set(game.env.snake)
    game.env.direction = DOWN
    game.move_queue = []
